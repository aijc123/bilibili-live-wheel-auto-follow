...
- **AI 陪聊推理模型适配**：用 MiMo-V2.5-Pro / DeepSeek-R1 / Qwen QwQ 等推理模型时，AI 陪聊频繁报「❌ LLM 调用失败：返回内容为空」——根因是写死的 `max_tokens: 300` 完全不够推理模型的 `reasoning_content` 思考消耗。现在默认抬到 32768（覆盖 DeepSeek-R1 硬上限、留够 MiMo 推理空间），网络 timeout 30s → 120s，`content` 为空时自动回退读 `reasoning_content`。设置面板新增「LLM token 上限」滑块（512–131072 可调）。
- **AI 陪聊角色直选**：4 个 persona（杠精 / 吐槽役 / 暖男 / 互动引导）此前藏在「设置 → LLM 提示词 → AI 陪聊（候选）」三层菜单深处；现在 AI 陪聊 section 顶部直接有「角色：[杠精 ▾]」下拉，一键换风格。
- **错误消息升级**：以前空响应只说「返回内容为空」用户没头绪；现在补一句「推理模型常见症状：max_tokens 不够，或思考过程没收敛到答案」直接给行动指引。
