...
- **新增「仅音频模式」**：右下角「弹幕助手」按钮旁多了「仅音频」开关。打开后停掉 B 站原生视频流，改成只听音频——带宽节省约 90%，多房间挂机时笔记本不烤机。视频流量大概率撑不住 5 个直播间同开，但音频可以。Cherry-pick 自 upstream chatterbox。
- **新增「AI 陪聊（候选）」**：同传 tab 底部多一段。配好 LLM key 之后，AI 听主播 STT + 房间弹幕，生成上下文相关的候选弹幕放队列里，**每条都要你点确认才发**。没有"自动发送"开关——LLM 输出有统计指纹，怕 B 站日后上 LLM 检测让你被 ban 更狠，这条线主动放弃。4 个 persona：杠精 / 吐槽役 / 暖男 / 互动派，自己挑。
